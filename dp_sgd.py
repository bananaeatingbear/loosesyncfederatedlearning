    ...
    for i in range(args.model_number):
        ...
        if (target_dataset.dataset_name == 'cifar100'):
            ...
            if (args.model_name == 'densenet_cifar'):
                target_model = convert_batchnorm_modules(densenet(num_classes=100))
            ...
            if (args.model_name == 'densenet_cifar'):
                target_model = convert_batchnorm_modules(densenet(num_classes=10))
            if (args.model_name=='resnet101'):
                model = models.resnet101(pretrained=args.pretrained)
                number_features = model.fc.in_features
        ...
        train, test, eval_attack,validation, eval_partition, in_train_partition, out_train_partition,starting_index = target_dataset.select_part(
        ...
        if (args.whitebox):
            ...
            if (args.middle_output):
                this_middle_output_train = get_middle_output(target_model,train,training_size=args.target_data_size)
                this_middle_output_test = get_middle_output(target_model,test,training_size=args.target_data_size)
            ...
            if (args.middle_gradient):
                #this_middle_gradient_train = get_middle_gradient(target_model, train,training_size=args.target_data_size,required_num=2000)
                #this_middle_gradient_test = get_middle_gradient(target_model, test,training_size=args.target_data_size,required_num=2000)

                this_middle_gradient_train = get_middle_gradient_one(target_model,train,training_size=args.target_data_size,required_num=400)
                this_middle_gradient_test = get_middle_gradient_one(target_model,test,training_size=args.target_data_size,required_num=400)
