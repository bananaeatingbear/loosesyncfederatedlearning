...
def clear_activation(model: nn.Module) -> None:
    for layer in model.modules():
...
def compute_grad1(model: nn.Module, loss_type: str = 'mean') -> None:
    ...
    for layer in model.modules():
        ...
        #print ("layer:",layer)
        #print ("layer activations:",A)
        #print ("layer activation shape:",A.shape)

        #print ("layer backprop list shape:",len(layer.backprops_list),layer.backprops_list[0].shape)
        ...
        if layer_type == 'Linear':
            ...
            A = torch.nn.functional.unfold(input=A, kernel_size=layer.kernel_size,padding=layer.padding,stride=layer.stride)
