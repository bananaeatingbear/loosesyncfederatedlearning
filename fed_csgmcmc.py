...
def assign_part_dataset(dataset, user_list=[]):
	...
	for i in range(len(user_list)):
		...
		user_list[i].validation_dataset = part_pytorch_dataset(valid_data, valid_label, train=False,
		                                                       transform=transform_test,
		                                                       target_transform=target_transform)
		user_list[i].validation_data_loader = torch.utils.data.DataLoader(user_list[i].validation_dataset,
...
# np.save(f'./csgmcmc/{dataset.dataset_name}_train_data.npy',dataset.train_data[assigned_index])
# np.save(f'./csgmcmc/{dataset.dataset_name}_train_label.npy',dataset.train_label[assigned_index])
# np.save(f'./csgmcmc/{dataset.dataset_name}_test_data.npy',dataset.test_data)
# np.save(f'./csgmcmc/{dataset.dataset_name}_test_label.npy',dataset.test_label)
...
def train_models(user_list, target_model, learning_rate, decay, epochs, target_dataset=None):
	...
	#	user_list[user_idx].scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(this_optim, T_max=epochs,eta_min=0.01*learning_rate)
		#user_list[user_idx].scheduler = torch.optim.lr_scheduler.CyclicLR(this_optim, base_lr=0.01*learning_rate, max_lr=learning_rate)
	...
	for cycle_index in range(num_cycles):
		...
		for user_idx in range(num_users):
			this_optim = torch.optim.SGD(user_list[user_idx].model.parameters(), lr=learning_rate, momentum=momentum,
			                             weight_decay=decay)
			user_list[user_idx].optim = this_optim
			user_list[user_idx].scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(this_optim, T_max=epochs,
		...
		for epoch in tqdm(range(epochs)):
			...
			for comm_round_idx in range(comm_round_per_epoch):
				...
				for idx in range(len(user_list)):
					...
					if (epoch+5<epochs):
						...
						train_data_loader = user_list[idx].create_new_train_data_loader(batch_size=args.target_batch_size)
						w = update_weights_mcmc(current_model_weights=ori_target_model_state_dict,
						                        model=user_list[user_idx].model, optimizer=user_list[user_idx].optim,
						                        train_loader=train_data_loader, local_epochs=args.local_epochs,
						                        mixup=args.mixup, num_step=args.num_step,
						                        class_weights=user_list[user_idx].class_weight, lr=user_list[0].scheduler.get_last_lr()[0]*0.1,
						                        datasize=args.target_data_size,alpha=1-momentum)
						user_list[idx].update_ban_list(ban_list)
						local_weights.append(copy.deepcopy(w))
			...
			train_acc, test_acc, valid_acc, train_loss, test_loss, valid_loss = get_train_test_acc(user_list,
			                                                                                       target_model,
			...
			if (valid_loss < min_loss):
				...
				for idx, this_model_weight in enumerate(local_weights):
					model_path = f"./model_checkpoints/cycle{cycle_index}_{args.model_name}_{args.dataset}_{args.num_step}_{args.user_number}_{args.target_data_size}_{idx}.pt"
					torch.save({'epoch': epoch, 'model_state_dict': target_model.state_dict(),
					            'optimizer_state_dict': user_list[idx].optim.state_dict()}, model_path)
	...
	train_acc, test_acc, train_loss, test_loss, ece_loss = get_train_test_acc(user_list, target_model, return_loss=True,
	                                                                          print_option=True, return_ece_loss=True)
